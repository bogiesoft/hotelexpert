...
<table width="986" align="center" border="0" cellpadding="0" cellspacing="0" style="border:1px #aaaaaa solid; margin-top:30px; border-top:none; border-radius:10px; ">
  ...
  <table width="966"  border="0" align="center" style="border:1px #9d9d9d solid; margin-top:10px; background-color:#efefef; border-radius:8px;">
    ...
    <ul class="tabs1">
		...
		<ul class="tabs">
		<?php if($this->session->userdata('hotel_room_inventory')==1) { ?>
        <li><a href="<?php echo WEB_URL;?>index/hotel_room_inventory/<?php echo $this->uri->segment(3); ?>/<?php echo $this->uri->segment(4); ?>" id="tabs_active" >
        Manage Inventory</a></li>
         <?php } ?>
          <?php if($this->session->userdata('hotel_pricing')==1) { ?>
        <li><a href="<?php echo WEB_URL;?>index/hotel_room_pricing/<?php echo $this->uri->segment(3); ?>/<?php echo $this->uri->segment(4); ?>"  style="width:200px;">
        Manage Pricing</a></li>
         <?php } ?>
...
<?php  echo '<b>Room Name:</b> '.$room_invenotory[0]->room_name.'</br>'; ?>
	    ...
	    <form method="post" action="<?php echo WEB_URL; ?>index/hotel_room_inventory/<?php echo $this->uri->segment(3); ?>/<?php echo $this->uri->segment(4); ?>">
			<div class="right_header" style="float:left;">
				...
				<a href="<?php echo WEB_URL; ?>index/hotel_room_inventory/<?php echo $this->uri->segment(3); ?>/<?php echo $this->uri->segment(4); ?>"><input type="button" value="Reset" style="width:90px; height:25px; background-color:#517ba5; color:#fff;border:1px solid #ccc;" /></a>
			</div>
        </form>
		<script>
		$(function() {
  ...
  <tr>
  	<?php if($this->session->userdata('hotel_room_inventory')==1) { ?>
    <td align="right" valign="top" class="" colspan="9" height="30"><a href="<?php echo site_url('index/add_hotel_room_inventory')?>/<?php echo $this->uri->segment(3); ?>/<?php echo $this->uri->segment(4); ?>" style="color:#2485BE; font-weight:bold;">Add Hotel Room Inventory</a>
    </td>
    <?php } ?>
  </tr>
   <tr>
    ...
    <td align="left" valign="top" class="my_profile_name_ex_tab">View Inventory</td>
    <?php if($this->session->userdata('hotel_room_inventory')==1) { ?>
...
<a href="<?php echo WEB_URL; ?>index/edit_hotel_room_inventory_details/<?php echo $this->uri->segment(3); ?>/<?php echo $this->uri->segment(4); ?>/<?php echo $result[$i]->sup_hotel_room_period_details_id; ?>" style="color:#099; "> <?php $fd = $result[$i]->room_avail_date_from; $fds = explode("-",$fd); $from_date = $fds[2].'-'.$fds[1].'-'.$fds[0]; $td = $result[$i]->room_avail_date_to; $tds = explode("-",$td); $to_date = $tds[2].'-'.$tds[1].'-'.$tds[0]; 
	...
	echo '<b>'.$from_date.'</b> To <b>'.$to_date.'</b>';	
     ...
     <td align="left" valign="top" class="my_profile_name_ex_tab_whit">
	<a href="<?php echo WEB_URL; ?>index/edit_hotel_room_inventory_details/<?php echo $this->uri->segment(3); ?>/<?php echo $this->uri->segment(4); ?>/<?php echo $result[$i]->sup_hotel_room_period_details_id; ?>" style="color:#2485BE;" >View Inventory</a>
    </td>
  ...
  <a href="<?php echo WEB_URL; ?>index/update_hotel_room_price_details/<?php echo $this->uri->segment(3); ?>/<?php echo $this->uri->segment(4); ?>/<?php echo $result[$i]->sup_hotel_room_period_details_id; ?>/1" style="color:#2485BE;" >Active</a> / 
   <a href="<?php echo WEB_URL; ?>index/update_hotel_room_price_details/<?php echo $this->uri->segment(3); ?>/<?php echo $this->uri->segment(4); ?>/<?php echo $result[$i]->sup_hotel_room_period_details_id; ?>/0" style="color:#2485BE;" >InActive</a> /-->
    ...
    <span style="font-size:12px;"><?php echo anchor(WEB_URL.'index/update_hotel_room_inventory_details/'.$this->uri->segment(3).'/'.$this->uri->segment(4).'/'. $result[$i]->sup_hotel_room_period_details_id.'/2', '<span style="color:#2485BE;">Delete</span>', array('onClick' => "return confirm('Are you sure you want to delete?')")); ?> </span> 
