      ...
      function(data) {
		if(data != ''){
			...
			for(var i=0; i<data.avail_dates.length; i++){
				$("#monthDates").append('<tr style="border-top:solid 1px #deab6f; border-bottom:solid 1px #deab6f; padding-bottom:2px; width: 108%;"><td  class="tdnew" id="tdtd">'+data.avail_dates[i].day+'-'+data.avail_dates[i].month+'-'+data.avail_dates[i].year+' '+data.avail_dates[i].week_day+'<input type="hidden" name="dates[]" value="'+data.avail_dates[i].date+'"><input type="hidden" name="weekday[]" value="'+data.avail_dates[i].week_day+'"></td><td class="tdnew" id="tdtd3"><input name="avilable_rooms[]" readonly="readonly" type="text" class="input-field"  style="height:20px;  background:#F2F2F2;" value="'+data.avail_dates[i].avilable_rooms+'" size="5" /></td><td class="tdnew" id="tdtd3"><input name="cost_price[]" type="text" class="input-field"  style="height:20px;  background:#F2F2F2;" value="'+data.avail_dates[i].cost_price+'" size="5"/></td><td class="tdnew" id="tdtd3"><input name="markup[]" type="text" class="input-field"  style="height:20px;  background:#F2F2F2;" value="'+data.avail_dates[i].markup+'" size="5"/></td><td  class="tdnew" id="tdtd3"><input name="selling_price[]" type="text" class="input-field"  style="height:20px;  background:#F2F2F2;" value="'+data.avail_dates[i].selling_price+'" size="5"/></td><td class="tdnew" id="tdtd3"><input type="text" name="discount_rule[]" id="discount_rule[]" class="input-field"  style="height:20px;  background:#F2F2F2;" value="'+data.avail_dates[i].discount_rule+'" size="5"/></td><td></td><td class="tdnew" id="tdtd3"><input name="final_price[]" type="text" class="input-field"  style=" height:20px;  background:#F2F2F2;" value="'+data.avail_dates[i].final_price+'" size="5"/></td><td class="tdnew" id="tdtd3">&nbsp;</td></tr>');
			}
		} 
	  }
	);
...
function getDates(){
	...
	$.post( "<?php print WEB_URL ?>index/getDates", {mmsd:sd1, mmed:ed1, rate_id:<?php echo $this->uri->segment(5); ?>, selectedDays:selectedDays, da:'days'},
      function(data) {
		if(data != ''){
			$("#filtering").show();
			...
			for(var i=0; i<data.dates.length; i++){
				...
				$("#monthDates").append('<tr style="border-top:solid 1px #deab6f; border-bottom:solid 1px #deab6f; padding-bottom:2px;"><td  class="tdnew" id="tdtd">'+data.dates[i]+'<input type="hidden" name="datess[]" value="'+day[0]+'"><input type="hidden" name="weekday[]" value="'+day[1]+'"></td><td  class="tdnew" id="tdtd3"><input name="avilable_rooms[]" type="text" class="input-field"  style="height:20px;  background:#F2F2F2;" size="5"/></td><td  class="tdnew" id="tdtd3"><input name="cost_price[]" type="text" class="input-field"  style="height:20px;  background:#F2F2F2;" size="5"/></td><td  class="tdnew" id="tdtd3"><input name="markup[]" type="text" class="input-field"  style="height:20px;  background:#F2F2F2;" size="5"/></td><td  class="tdnew" id="tdtd3"><input  type="text" name="selling_price[]" class="input-field"  style="height:20px;  background:#F2F2F2;" size="5"></td><td  class="tdnew" id="tdtd3"><input type="text" name="discount_rule[]" id="discount_rule[]" class="input-field"  style="height:20px;  background:#F2F2F2;" size="5"></td><td></td><td  class="tdnew" id="tdtd3"><input name="final_price[]" type="text" class="input-field"  style="height:20px;  background:#F2F2F2;" size="5"/></td><td  class="tdnew" id="tdtd3"></tr>');
			}
		} 
	  }
	);
  ...
  <table width="966"  border="0" align="center" style="border:1px #9d9d9d solid; margin-top:10px; background-color:#efefef; border-radius:8px;">
    ...
    <ul class="tabs1">
      ...
      <div class="rightpartbox" style="margin:0px;">
        ...
        <div style="float:right; color:#517BA5;"><a href="<?php echo WEB_URL; ?>index/hotel_room_pricing/<?php echo $this->uri->segment(3);  ?>/<?php echo $this->uri->segment(4);  ?>">Go Back</a></div>
        <div style="float:left; margin-left:20px;">
      
    
     <form name="form1" action="<?php echo WEB_URL; ?>index/update_room_price/<?php echo $this->uri->segment(3); ?>/<?php echo $this->uri->segment(4); ?>/<?php echo $this->uri->segment(5); ?>" method="post">
        <table width="100%" border="0" cellspacing="1" cellpadding="0">
        	<tr><td>&nbsp;</td></tr>
        	 <tr height="35">
            <td width="30%"> Room Type <font style="margin-left: 3px;" color="red">*</font></td>
            <td width="77%"> <input readonly="readonly" type="text" name="room_category" value="<?php echo $this->admin_model->get_avail_category_type($room_invenotory[0]->global_room_category_type_id); ?>" />
            </td>
            </tr>
   ...
   <tr>
            ...
            <tr height="35">
                <td colspan="2" style="padding-top:20px;"><b>Manage Pricing</b></td>
            </tr>
        </table>
    <!--</form>   
    ...
    <td colspan="2" style="border-bottom:solid 0px #666; display:none;"><strong>Day</strong> <span style="float:right;"><input name="all_day" id="all_day" type="checkbox" value="" onclick="return check_all();" /> <strong>Check All</strong></span>
    ...
    <td style="width:250px;"> Room Available Dates:<font style="margin-left: 3px;" color="red">*</font>
        ...
        <?php
			...
			$fm = $hotel_room_details->room_avail_date_from;
				$fmd = explode("-",$fm);
				$fdate = $fmd[2].'-'.$fmd[1].'-'.$fmd[0];
				$td = $hotel_room_details->room_avail_date_to;
				$tda = explode("-",$td);
				$tdate = $tda[2].'-'.$tda[1].'-'.$tda[0];
        ?>
    <td style="width:120px;"><input name="sd"  readonly="readonly" value="<?php echo $fdate; ?>"  onchange="javascript:return check_night_valued(this.value)" type="text" class="b2b-txtbox"  style=" background-image: url('<?php echo WEB_DIR_FRONT; ?>images/b2b-calicon.png');background-repeat:no-repeat;background-position:right center;width:100px; height:18px;"/></td>
    <td >&nbsp;</td>
    <td  style="width:120px;"><span id="out"><input name="ed" value="<?php echo $tdate; ?>"  readonly="readonly" onchange="javascript:return check_night_valued(this.value)" type="text" class="b2b-txtbox" style=" background-image: url('<?php echo WEB_DIR_FRONT; ?>images/b2b-calicon.png'); background-repeat:no-repeat;background-position:right center;width:100px; height:18px;"/></span></td>
     <td style="width:20px;">
  
      </td>
    <td colspan="3" id="day" style="display:none;">
    ...
    <td colspan="2"><input name="" type="hidden"  value="submit" onclick="getDates();"/></td>
    </tr>
    </table>
    ...
    <td style="border-right:solid 1px #deab6f; text-align:center; padding-left:0px;"><input name="avilable_rooms" readonly="readonly" id="avilable_rooms" type="text" class="input-field"  style="height:20px; margin-top:3px; background:#F2F2F2;" size="5"/></td>
    ...
    <td style="border-right:solid 1px #deab6f; text-align:center; padding-left:0px;"><input name="markup" id="markup" type="text" class="input-field"  style="height:20px; margin-top:3px; background:#F2F2F2;" size="5"/></br>
      <input type="radio" name="check_markup" value="0"<?php if($hotel_room_details->check_markup == 0)  echo "checked='checked'";?>/><font color="#FFFFFF" >%</font> &nbsp;<input type="radio" name="check_markup" value="1"<?php if($hotel_room_details->check_markup == 1) echo "checked='checked'";?> /><font color="#FFFFFF" >$</font></td>
    ...
    <td style="border-right:solid 1px #deab6f; text-align:center; padding-left:0px;">
     ...
     <input type="radio" name="check_discount" value="0"<?php if($hotel_room_details->check_discount == 0) echo "checked='checked'";?>  /><font color="#FFFFFF" >%</font> &nbsp;<input type="radio" name="check_discount" value="1"<?php if($hotel_room_details->check_discount == 1) echo "checked='checked'";?> /><font color="#FFFFFF" >$</font></td>
