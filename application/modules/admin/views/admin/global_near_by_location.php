...
<table width="986" align="center" border="0" cellpadding="0" cellspacing="0" style="border:1px #aaaaaa solid; margin-top:30px; border-top:none; border-radius:10px; ">
  ...
  <table width="966"  border="0" align="center" style="border:1px #9d9d9d solid; margin-top:10px; background-color:#efefef; border-radius:8px;">
    ...
    <ul class="tabs1">
        <li><a href="<?php echo WEB_URL;?>index/global_near_by_location"  id="tabs_active" style="width:200px;">Near By Location</a></li>
       </ul>
 ...
 <div  class="right_header"><strong><a href="<?php echo WEB_URL;?>index/dashboard">Go back</a></strong></div>
        ...
        <div class="tablebox" style="width:910px; margin:0px">
         ...
         <form name="f2" action="<?php echo WEB_URL;?>index/add_near_by_location" method="post">
		<table width="400">
			<tr>
				<td> Add Near By Location</td>
			
				<td><input type="text" name="category_type" id="category_type" required></td>
		...
		$location= $this->admin_model->get_global_near_by_location();
        if (!empty($location)) {
        for($i=0;$i< count($location);$i++) { ?>
        <div class="tb_sample_cover211" style="width:600px">
        	 <div class="tr_col_01  bl_2" style="width:140px;"><?php echo $i+1; ?></div>
            <div class="tr_col_01  bl_2" style="width:140px;"><?php echo $location[$i]->location_name; ?></div>
            <div class="tr_col_01 bl_1" style="width:140px;"><?php if($location[$i]->status==1) { echo "Active";}else {echo "Inactive";} ?></div>
            <div class="tr_col_01 bl_1" style="width:175px;">  
          	 <a href="<?php echo WEB_URL;?>index/edit_near_by_location/<?php echo $location[$i]->global_near_by_location_id; ?>">Edit</a> /
             <a href="<?php echo WEB_URL;?>index/update_near_by_location/<?php echo $location[$i]->global_near_by_location_id; ?>/1">Active</a> /
             <a href="<?php echo WEB_URL;?>index/update_near_by_location/<?php echo $location[$i]->global_near_by_location_id; ?>/0">InActive</a>/
              <span style="font-size:11px;"><?php echo anchor(WEB_URL.'index/update_near_by_location/'.$location[$i]->global_near_by_location_id .'/2', '<span style="color:#2485BE;">Delete</span>', array('onClick' => "return confirm('Are you sure you want to delete?')")); ?> </span>
            <!-- <a href="<?php echo WEB_URL;?>index/update_room_hotel_facility/<?php echo $location[$i]->global_near_by_location_id; ?>/2">Delete</a>-->
