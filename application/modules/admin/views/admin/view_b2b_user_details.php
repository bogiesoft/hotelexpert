  ...
  <table width="966"  border="0" align="center" style="border:1px #9d9d9d solid; margin-top:10px; background-color:#efefef; border-radius:8px;">
	...
	<div class="tabs_width">
     <ul class="tabs1">
        ...
        <li><a href="<?php echo WEB_URL;?>home/view_b2b_user_details" id="tabs_active" >Manage B2B </a></li>
        <?php } ?>
...
<?php if($this->session->userdata('add_travel_agents')==1) { ?>
<div class="right_header" style="float:right;"><a href="<?php echo WEB_URL;?>home/add_b2b_user" style="color:#2485BE;">Add B2B User</a></div>
<?php } ?>
	<form method="post" action="<?php echo WEB_URL; ?>home/view_b2b_user_details">
		<div class="right_header" style="float:left;">
			...
			<a href="<?php echo WEB_URL; ?>home/view_b2b_user_details"><input type="button" value="Reset" style="width:90px; height:25px; background-color:#517ba5; color:#fff;border:1px solid #ccc;" /></a>
		</div>
	</form>
  ...
  <tr>
    ...
    <td align="center" class="admin-table-colo">Contact No</td>
     <td align="center" class="admin-table-colo">status</td>
      <?php if($this->session->userdata('edit_travel_agents')==1 || $this->session->userdata('delete_travel_agents')==1){ ?>
...
for($i=0;$i<count($sup_details);$i++)
{
   ...
   <tr >
	...
	<?php if(isset($sup_details[$i]->supplier_logo) && $sup_details[$i]->user_logo!='' ) {?> <img src="<?php echo WEB_DIR_FRONT ?>supplier_logos/<?php echo $sup_details[$i]->supplier_logo; ?>" width="20" height="20" border="0" /> <?php  } ?>
	
	<?php echo $sup_details[$i]->name;  ?></td>
    ...
    <td align="center" class="admin-table-colo1">
     ...
     <?php if($this->session->userdata('edit_travel_agents')==1) { ?>
     <a href="<?php echo WEB_URL; ?>home/edit_b2b_user/<?php echo $sup_details[$i]->user_id ; ?> " style="color:#2485BE;">Edit</a> /
      <?php } ?>
       <?php if($this->session->userdata('delete_travel_agents')==1) { ?>
    <a href="<?php echo WEB_URL; ?>home/update_b2b_user/<?php echo $sup_details[$i]->user_id; ?>/1 " style="color:#2485BE;">Active</a> / 
    <a href="<?php echo WEB_URL; ?>home/update_b2b_user/<?php echo $sup_details[$i]->user_id; ?>/0" style="color:#2485BE;">InActive</a>/ 
      <span style="font-size:12px;"><?php echo anchor(WEB_URL.'home/update_b2b_user/'.$sup_details[$i]->user_id.'/2', '<span style="color:#2485BE;">Delete</span>', array('onClick' => "return confirm('Are you sure you want to delete?')")); ?> </span>
      
       <?php } ?>
   <!-- <a href="<?php echo WEB_URL; ?>home/update_sup_user/<?php echo $sup_details[$i]->user_id; ?>/2">Delete</a>--></td>
