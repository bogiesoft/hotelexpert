	...
	<li><a  href="<?php echo WEB_URL ?>index/view_supplier_details" style="width:200px;" >Edit/ Delete Supplier User</a></li>
     <?php } ?>  
      ...
      <div class="rightpartbox">
        ...
        <?php if($this->session->userdata('add_supplier_type')==1) { ?>
        <div class="tablebox"  >
        <form method="post" name="admin_type" action="<?php echo WEB_URL; ?>index/add_supplier_type" enctype="multipart/form-data">
          ...
          <table width="100%" align="left" border="0" cellspacing="1" cellpadding="5" style="margin-left:10px;">
            ...
            <tr>
              ...
              <td class="colo-td"><input type="text" name="supplier_type" size="30" value="<?php echo set_value('supplier_type'); ?>" required /></td>
               <td class="colo-td" align="center" colspan="2"><input type="submit"value="Create supplier Type"  
...
<table align="right" width="100%" border="0" cellspacing="0" cellpadding="0" style="height:20px;">
  ...
  <tr>
    ...
    <td align="center" class="admin-table-colo">Status</td>
     <?php if($this->session->userdata('edit_supplier_type')==1 || $this->session->userdata('delete_supplier_type')==1  ) { ?>
...
for($i=0;$i<count($sup_details);$i++)
{
   ...
   <tr >
    ...
    <td align="center" class="admin-table-colo1"><?php echo $sup_details[$i]->supplier_type; ?></td>
    ...
    <?php if($this->session->userdata('edit_supplier_type')==1) { ?>
    <a href="<?php echo WEB_URL; ?>index/edit_supplier_type/<?php echo $sup_details[$i]->supplier_type_id; ?> ">Edit</a> / 
     <?php } ?>
     <?php if($this->session->userdata('delete_supplier_type')==1) { ?>
    <a href="<?php echo WEB_URL; ?>index/update_supplier_type/<?php echo $sup_details[$i]->supplier_type_id; ?>/1 ">Active</a> / 
    <a href="<?php echo WEB_URL; ?>index/update_supplier_type/<?php echo $sup_details[$i]->supplier_type_id ; ?>/0">InActive</a>/
      <span style="font-size:12px;"><?php echo anchor(WEB_URL.'index/update_supplier_type/'.$sup_details[$i]->supplier_type_id.'/2', '<span style="color:#2485BE;">Delete</span>', array('onClick' => "return confirm('Are you sure you want to delete?')")); ?> </span>
      <?php } ?>
    <!--<a href="<?php echo WEB_URL; ?>index/update_admin_type/<?php echo $sup_details[$i]->supplier_type_id ; ?>/2">Delete</a>-->
    </td>
     <?php } ?>
  </tr>
 <?php
