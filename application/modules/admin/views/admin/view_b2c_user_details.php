...
<div id="containerdount">
  <?php if($this->session->userdata('add_b2c_customer')==1) { ?>
<div class="right_header" style="float:right;"><a href="<?php echo WEB_URL;?>index/add_b2c_user" style="color:#2485BE;">Add B2C User</a></div>
<?php } ?>
	<form method="post" action="<?php echo WEB_URL; ?>index/view_b2c_user_details">
		<div class="right_header" style="float:left;">
			...
			<a href="<?php echo WEB_URL; ?>index/view_b2c_user_details"><input type="button" value="Reset" style="width:90px; height:25px; background-color:#517ba5; color:#fff;border:1px solid #ccc;" /></a>
		</div>
	</form>
<table width="98%" border="0" cellspacing="1" cellpadding="0" style="background-color:#E8E5E5; margin:21px 0 0 8px" >
  ...
  <tr>
    ...
    <td align="center" class="admin-table-colo">Contact No</td>
     ...
     <?php if($this->session->userdata('delete_b2c_customer')==1 || $this->session->userdata('edit_b2c_customer')==1){ ?>
...
for($i=0;$i<count($sup_details);$i++)
{
   ...
   <tr >
	...
	<?php if(isset($sup_details[$i]->b2c_user_logo) && $sup_details[$i]->b2c_user_logo!='' ) {?> <img src="<?php echo WEB_DIR_FRONT ?>b2c_user_logo/<?php echo $sup_details[$i]->b2c_user_logo; ?>" width="20" height="20" border="0" /> <?php  } ?>
    ...
    <td align="center" class="admin-table-colo1"><?php if($sup_details[$i]->status==1) { echo 'Active'; } else { echo 'InActive'; } ?></td>
     ...
     <?php if($this->session->userdata('delete_b2c_customer')==1 || $this->session->userdata('edit_b2c_customer')==1){ ?>
    <td align="center" class="admin-table-colo1">
    <?php if($this->session->userdata('edit_b2c_customer')==1) { ?>
     <a href="<?php echo WEB_URL; ?>index/edit_b2c_user/<?php echo $sup_details[$i]->user_id ; ?> " style="color:#2485BE;">Edit</a> /
     <?php } ?>
     <?php if($this->session->userdata('delete_b2c_customer')==1) { ?>
    <a href="<?php echo WEB_URL; ?>index/update_b2c_user/<?php echo $sup_details[$i]->user_id ; ?>/1 " style="color:#2485BE;">Active</a> / 
    <a href="<?php echo WEB_URL; ?>index/update_b2c_user/<?php echo $sup_details[$i]->user_id ; ?>/0" style="color:#2485BE;">InActive</a>/ 
      <span style="font-size:12px;"><?php echo anchor(WEB_URL.'index/update_b2c_user/'.$sup_details[$i]->user_id .'/2', '<span style="color:#2485BE;">Delete</span>', array('onClick' => "return confirm('Are you sure you want to delete?')")); ?> </span>
      <?php } ?>
 	</td>
     <?php } ?>
